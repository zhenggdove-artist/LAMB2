...
type Phase = 'drawing' | 'game' | 'gameover';
...
// viewType: 'side' (Looks +X, Wedge) OR 'front' (Looks +Z, Almond)
const generateCyberEye = (THREE: any, viewType: 'side' | 'front') => {
    ...
    const addPoint = (x1:number, y1:number, z1:number, x2:number, y2:number, z2:number, s:number, c:any) => {
...
const PointCloudHealthBar = ({ health, lastHit }: { health: number, lastHit: number }) => {
...
// ==========================================
const DrawingPhase = ({ onFinish }: { onFinish: (points: number[]) => void }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
...
// PHASE 2: THE GAME
// ==========================================
const GamePhase = ({ pointData, onGameOver }: { pointData: number[], onGameOver: () => void }) => {
  ...
  type EyeState = {
      mesh: any;
      uniforms: { uBlink: { value: number } };
      nextBlinkTime: number;
      blinkState: 'open' | 'closing' | 'closed' | 'opening';
      blinkSpeed: number; // Slight variation
  };
  const eyesRef = useRef<EyeState[]>([]);
  ...
  useEffect(() => {
    ...
    const createEye = (type: 'side' | 'front') => {
        let px=0, py=0, pz=0;
    ...
    const createSingleTentacle = (seedIndex: number, initialScale: number) => {
        // =================================================
...
const App = () => {
  ...
  const [pointData, setPointData] = useState<number[]>([]);

  const handleDrawFinish = (points: number[]) => {
    setPointData(points);
    setPhase('game');
  };
